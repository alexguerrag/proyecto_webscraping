...
def scrape_ml2(url, min_discount):
    ...
    for product in products:
        ...
        old_price_tag = product.find('s', {'class': 'andes-money-amount-combo__previous-value'}).find('span', {'class': 'andes-money-amount__fraction'}).text.strip()
        
        
        if old_price_tag is not None:
            old_price = old_price_tag.find('s', {'class': 'andes-money-amount-combo__previous-value'}).find('span', {'class': 'andes-money-amount__fraction'}).text.strip()
        else:
            old_price = price
